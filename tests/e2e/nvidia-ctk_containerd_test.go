...
var _ = Describe("containerd", Ordered, ContinueOnFailure, func() {
	var (
		...
		containerName       = "nvctk-e2e-nvidia-ctk-containerd-tests"
	)

	BeforeAll(func(ctx context.Context) {
		var err error
		nestedContainerRunner, err = NewNestedContainerRunner(runner, outerContainerImage, installCTK, containerName, localCacheDir)
